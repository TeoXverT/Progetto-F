...
/**
 * Pannello che tramite la sua interfaccia permette di sciegleire i a sedere posti al interno di una sala e contemporaneamente visualizza un sub-totale
 *
*/

public class PanelSeatSelection extends JPanel {
    private CustomerController controller;
    private double totale_prezzo;
    private Component frameErrore;
    private JLabel prezzo = new JLabel();
    
    public PanelSeatSelection(Projection proiezione) {
    ...
    private void initGui(Projection screening) {
        try {
            ...
            Logger.getLogger(PanelSeatSelection.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void cartControl(ArrayList<Seat> Taken_seats, Booking booking, Projection screening) {  //  questo metodo ha il compito di controllare 
        if (Taken_seats.size() != 0) {
            ...
            openPage(new PanelCheckout(booking));
        } else {
    ...
    private void goback() {  //ritorna alla pagina selezione film
        this.removeAll();
        this.add(new PanelDayAndFilmSelection());
        this.revalidate();
        this.repaint();
    }
